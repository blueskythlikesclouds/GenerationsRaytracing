...
#undef ISBITMASK
        ...
        _Outptr_ D3D12MA::Allocation** allocation) noexcept
    {
        ...
        if (SUCCEEDED(hr))
        {
            assert(allocation != nullptr && *allocation != nullptr);
            _Analysis_assume_(allocation != nullptr && *allocation != nullptr);

            SetDebugObjectName((*allocation)->GetResource(), L"DDSTextureLoader");
        }
    ...
    bool* isCubeMap)
{
    ...
    if (!d3dDevice || !ddsData || !allocation)
    {
        return E_INVALIDARG;
    }
    ...
    bool* isCubeMap)
{
    ...
    if (!d3dDevice || !fileName || !allocation)
    {
        return E_INVALIDARG;
    }
