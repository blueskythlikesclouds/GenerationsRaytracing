...
else
{
    ...
    foreach (var (shaderName, vertexShader, pixelShader, shaderMapping) in shaderLists)
    {
        ...
        RaytracingShaderConverter.WriteRaytracingFunctions(stringBuilder, raytracingFunctionName, shaderIndex, vertexShader, pixelShader, shaderMapping);

        shaderMappingWriter.Write(shaderName);
        shaderMappingWriter.Write(shaderName == raytracingFunctionName ? string.Empty : raytracingFunctionName);
