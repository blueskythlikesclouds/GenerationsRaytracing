...
bool EnvironmentColor::get(const Bridge& bridge, float* result)
{
    ...
    hash = XXH64(bridge.psConstants.c[10], 12, hash);
    hash = XXH64(bridge.psConstants.c[36], 12, hash);

    const auto pair = ENVIRONMENT_COLORS.find(hash);
    if (pair == ENVIRONMENT_COLORS.end())
        return false;
    ...
    result[0] = static_cast<float>(pair->second[0]) / 255.0f;
    result[1] = static_cast<float>(pair->second[1]) / 255.0f;
    result[2] = static_cast<float>(pair->second[2]) / 255.0f;
